...
RUL_01 = np.loadtxt('./data/RUL_FD004.txt')
train_01_raw = np.loadtxt('./data/train_FD004.txt')
test_01_raw = np.loadtxt('./data/test_FD004.txt')
...
for i in range(10):  # run 10 times
    ...
    scipy.io.savemat('results/proposed' + str(i + 1) + '_FD004.mat', dict(y_true=testTarget, y_pred=yPreds))
    K.clear_session()
